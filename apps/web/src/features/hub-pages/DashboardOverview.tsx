...
function WeeklyStatsCard({ stats }: WeeklyStatsCardProps) {
  const statItems = [
    { label: 'Training days', value: stats.trainingDays, icon: Calendar, color: 'text-green-600', bg: 'bg-green-50' },
    { label: 'Upcoming tests', value: stats.upcomingTests, icon: Target, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Weekly goal', value: `${stats.weeklyGoalProgress}%`, icon: CheckCircle, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Badges earned', value: stats.badges, icon: Trophy, color: 'text-purple-600', bg: 'bg-purple-50' },
  ];
...
function TodayFocusCard({ todayState, isLoading }: TodayFocusCardProps) {
  if (isLoading) {
    return (
      <div className="bg-white rounded-xl p-6 shadow-sm border border-tier-border-default animate-pulse">
      ...
      <div className="space-y-3">
        {focusItems.map((item, index) => (
          <Link
            key={index}
            to={item.href}
            className="group flex items-center gap-3 p-3 rounded-lg hover:bg-tier-surface-subtle transition-colors"
          >
